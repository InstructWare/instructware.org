...
7. **MUST** 满足第 9.1 节的实现可追溯契约要求，以及第 10.1 节按剖面生效的漂移控制门禁要求。
...
- **Intent Drift（意图漂移）:** 运行时行为或实现产物不再忠实反映当前源意图文档的状态。
...
- 在多目标场景下，实现 **SHOULD** 采用“共享优先 + 目标覆盖”的组织方式，以减少重复意图文档。
...
- 实现 **MUST NOT** 要求“仅允许单一自然语言编写意图”或“仅允许单一实现语言承载执行层”。
- 实现 **MUST** 在所有已声明支持的意图语言与运行时语言目标上，执行一致的可追溯与漂移控制门禁（第 9.1 与第 10.1 章）。
- 实现 **SHOULD** 发布意图语言与运行时语言目标的支持矩阵，并给出明确成熟度标签（例如 `stable`、`beta`、`experimental`）。
...
1. 实现 **MUST** 定义并文档化链接策略，将节点类别（如 kind、关键性或 section）映射到必需的链接覆盖行为。
...
3. 意图覆盖阈值（由实现定义并文档化，允许按节点类别与剖面差异化）；
4. 受影响节点的回归检查。
...
IWP 与许多传统接口规范的关键差异在于：它将“意图到执行”的可追溯性与漂移控制提升为协议级要求，而非可选工具约定。
...
6. 记录能力调用相关的指令来源与策略决策，支持审计追踪。
7. 高风险动作（如特权写入或外部副作用）要求显式审批检查点。
...
该剖面 **MAY** 同样用于多目标项目，按需采用共享资产与目标覆盖结构。
